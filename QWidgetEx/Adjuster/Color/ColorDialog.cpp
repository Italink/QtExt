...
void ColorDialog::setAlphaEnabled(bool a)
{
    ...
    {
        p->alpha_enabled = a;
        p->ui.preview->setDisplayMode(a ? ColorPreview::AllAlpha : ColorPreview::NoAlpha);
...
void ColorDialog::setCurrentColorInternal(const QColor& col)
{
    /**
...
void ColorDialog::on_buttonBox_clicked(QAbstractButton* btn)
{
    ...
    case QDialogButtonBox::ResetRole:
        // Restore old color
        setCurrentColorInternal(p->ui.preview->comparisonColor());
        break;
...
void ColorDialog::dropEvent(QDropEvent* event)
{
    if (event->mimeData()->hasColor())
    {
        setCurrentColorInternal(event->mimeData()->colorData().value<QColor>());
        event->accept();
    }
...
void ColorDialog::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    {
        setCurrentColorInternal(utils::get_screen_color(event->globalPosition().toPoint()));
